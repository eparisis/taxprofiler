...
- `motus/`
  - `<db_name>/`
    ...
    - `<sample_id>.prep_long.gz`: Output file of long reads that have cut up into into shorter reads.
...
- `sylph/`
  ...
  - `sylph_<db_name>_combined_reports`: output of `sylph-tax merge` containing merged multiple taxonomic profiles into a TSV table.
  </details>
